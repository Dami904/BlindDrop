...
export function useAutoSend(recipients: AutoSendRecipient[]) {
  ...
  // Load any previously-saved config on mount (client-only — localStorage
  // isn't available during server rendering).
  useEffect(() => {
    const saved = loadEmailJsConfig();
    if (saved) {
      setConfig(saved);
      setDraft(saved);
    }
            ...
            <span className="min-w-0 flex-1">
              ...
              {!open && (
                <span className="mt-1 block text-xs" style={{ color: "var(--text-dim)" }}>
                  Bulk-send {total} claim link{total === 1 ? "" : "s"} through your own EmailJS account.
                </span>
              )}
            </span>
